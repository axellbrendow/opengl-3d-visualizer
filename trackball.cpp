...
void TrackBall :: mouseMove ( const QPointF &p)
{
    ...
    if ( msecs ) {
        ...
        if (lenSqr >= 1.0) currentPos3D.normalize();
        else currentPos3D.setZ(sqrt(1.0 - lenSqr));
...
void TrackBall :: mousePress ( const QPointF &p)
{
    ...
    if (lenSqr >= 1.0) lastPos3D . normalize ();
    else lastPos3D . setZ ( sqrt (1.0 - lenSqr ));
